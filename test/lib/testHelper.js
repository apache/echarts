...
(function (context) {
    ...
    function initButtons(chart, opt, buttonsContainer, errMsgPrefix) {
        ...
        for (var i = 0; i < buttons.length; i++) {
            var created = createInputByDefine(buttons[i]);
            for (var j = 0; j < created.length; j++) {
                buttonsContainer.appendChild(created[j]);
            }
        }
        ...
        function createInputByDefine(btnDefine) {
            if (!btnDefine) {
                return;
            }
        ...
        function createRangeInput(btnDefine, internallyForceDef) {
            ...
            var sliderListener = internallyForceDef
                ...
                : getBtnEventListener(btnDefine, NAMES_ON_INPUT_CHANGE);
            if (!sliderListener) {
            ...
            function resetWrapperCSS(disabled) {
                sliderWrapperEl.className = 'test-buttons-slider'
                    + (internallyForceDef ? ' test-buttons-slider-sub' : '')
                    + (disabled ? ' test-buttons-slider-disabled' : '');
            }
            ...
            return {
                ...
                getValue: function () {
                    return sliderInputEl.value;
                }
            };
        }

        function createSelectInput(btnDefine) {
            var result = [];
            var optionIdxToSubInput = [];

            var selectWrapperEl = document.createElement('span');
            selectWrapperEl.className = 'test-buttons-select';
            result.push(selectWrapperEl);
            ...
            function disableSubInputs(currOptionIdx) {
                for (var i = 0; i < optionIdxToSubInput.length; i++) {
                    var subInput = optionIdxToSubInput[i];
                    if (subInput) {
            ...
            if (btnDefine.options) {
                for (var optionIdx = 0; optionIdx < btnDefine.options.length; optionIdx++) {
                    var optionDef = btnDefine.options[optionIdx];
                    if (
                        !isObject(optionDef)
                        || (
            ...
            else if (btnDefine.values) {
                for (var optionIdx = 0; optionIdx < btnDefine.values.length; optionIdx++) {
                    var value = btnDefine.values[optionIdx];
                    optionDefList.push({
                        value: value,
            ...
            for (var optionIdx = 0; optionIdx < optionDefList.length; optionIdx++) {
                var optionDef = optionDefList[optionIdx];
                optionDefList[optionIdx] = optionDef;
                ...
                if (optionDef.input) {
                    ...
                    result.push(createdRangeInput.el);
                    optionIdxToSubInput[optionIdx] = createdRangeInput;
                }
            }
            ...
            if (btnDefine.hasOwnProperty('valueIndex')) {
                ...
                for (var idx = 0; idx < optionDefList.length; idx++) {
                    if (!optionDefList[idx].input && optionDefList[idx].value === btnDefine.value) {
                        selectEl.value = idx;
                        found = true;
                        initOptionIdx = idx;
                    }
            ...
            selectEl.addEventListener('change', function () {
                var optionIdx = +this.value;
                disableSubInputs(optionIdx);
                var value = optionDefList[optionIdx].input
                    ? optionIdxToSubInput[optionIdx].getValue()
                    : optionDefList[optionIdx].value;
                handleSelectChange(value);
            });

            disableSubInputs(initOptionIdx);

            return result;
        }

        function createBr(btnDefine) {
            return document.createElement('br');
        }

        function createButtonInput(btnDefine) {
