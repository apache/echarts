...
class SeriesModel<Opt extends SeriesOption = SeriesOption> extends ComponentModel<Opt> {
    ...
    private _innerSelect(data: SeriesData, innerDataIndices: number[]) {
        ...
        if (selectedMode === 'series') {
            ...
            const self = this;
            data.each(idx => {
                if ((data.getItemModel(idx) as Model).get(['select', 'disable']) === true) {
                    return;
                }
                const nameOrId = getSelectionKey(data, idx);
            ...
            for (let i = 0; i < len; i++) {
                const dataIndex = innerDataIndices[i];
                if ((data.getItemModel(dataIndex) as Model).get(['select', 'disable']) === true) {
                    return;
                }
            ...
            if ((data.getItemModel(lastDataIndex) as Model).get(['select', 'disable']) === true) {
                return;
            }
