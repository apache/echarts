...
class PiecewiseModel extends VisualMapModel<PiecewiseVisualMapOption> {
  ...
  completeVisualOption() {
    ...
    zrUtil.each(
      visualTypesInPieces,
      function (v, visualType: BuiltinVisualProperty) {
        let exists = false;
        zrUtil.each(
          this.stateList,
        ...
        !exists
          && zrUtil.each(this.stateList, function (state: VisualState) {
            (option[state] || (option[state] = {}))[visualType] =
              visualDefault.get(
                visualType,
    ...
    function has(
      obj: PiecewiseVisualMapOption['target'],
      state: VisualState,
      visualType: BuiltinVisualProperty
    ) {
  ...
  private _resetSelected(
    newOption: PiecewiseVisualMapOption,
    isInit?: boolean
  ) {
...
function normalizeReverse(
  thisOption: PiecewiseVisualMapOption,
  pieceList: InnerVisualPiece[]
) {
