...
define(function (require) {
    ...
    DataZoom.prototype = {
        ...
        _setLimits: function(xStart, xEnd, yStart, yEnd) {
            var xAxis = this.option.xAxis;
            var yAxis = this.option.yAxis;
            var positive = function(min, max) {
                if (min >= 0) { return true; }
                if (min < 0 && max < 0) { return false; }
            };

            function pow(x, sign, base) {
                return sign ? Math.pow(base, x) : -Math.pow(base, -x);
            }

            for (var i = 0; i < xAxis.length; i++) {
                if (xAxis[i].type === 'logarithm') {
                    var signX = positive(xStart, xEnd);

                    xStart = pow(xStart, signX, this.option.xAxis[i].logOptions.base);
                    xEnd = pow(xEnd, signX, this.option.xAxis[i].logOptions.base);
                }
            }

            for (var j = 0; j < yAxis.length; j++) {
                if (yAxis[j].type === 'logarithm') {
                    var signY = positive(yStart, yEnd);

                    yStart = pow(yStart, signY, this.option.yAxis[j].logOptions.base);
                    yEnd = pow(yEnd, signY, this.option.yAxis[j].logOptions.base);
                }
            }
        ...
        _synScatterData : function (seriesIndex, data) {
            ...
            limits = this._setLimits(xStart, xEnd, yStart, yEnd);
            xStart  = limits[0];
            xEnd    = limits[1];
            yStart  = limits[2];
            yEnd    = limits[3];
