...
type AxisInnerStore = {
  axisTick: AxisCache<
    AxisCategoryTickLabelCacheKey<'axisTick'>,
    AxisCategoryTickCreated
  >;
  axisLabel: AxisCache<
    AxisCategoryTickLabelCacheKey<'axisLabel'>,
...
export function createAxisLabelsComputingContext(
  kind: AxisTickLabelComputingKind
...
export function createAxisLabels(
  ...
  if (custom) {
    ...
    const ticks = zrUtil.filter(
      tickNumbers,
      (val) => val >= extent[0] && val <= extent[1]
    );
    return {
...
function makeRealNumberLabels(axis: Axis): ReturnType<typeof createAxisLabels> {
  ...
  const pruneByBreak =
    includeMinLabel || includeMaxLabel ? 'preserve_extent_bound' : 'auto';
  ...
  if (includeMinLabel && ticks.length > 0) {
    ...
    if (notAtExtentStart || isFirstTickBreak) {
      ...
      const timeInfo =
        firstTick.time && !isFirstTickBreak ? firstTick.time : undefined;
      labels.unshift({
  ...
  if (includeMaxLabel && ticks.length > 0) {
    ...
    if (notAtExtentEnd || isLastTickBreak) {
      ...
      const timeInfo =
        lastTick.time && !isLastTickBreak ? lastTick.time : undefined;
      labels.push({
 ...
 */
function initAxisCacheMethod<TCacheProp extends AxisInnerStoreCacheProp>(
  prop: TCacheProp
) {
...
function axisCacheGet<TKey, TVal>(
  cache: AxisCache<TKey, TVal>,
  key: TKey
): TVal | NullUndefined {
...
function axisCacheSet<TKey, TVal>(
  cache: AxisCache<TKey, TVal>,
  key: TKey,
  value: TVal
): TVal {
 ...
 */
export function calculateCategoryInterval(
  axis: Axis,
  ctx: AxisLabelsComputingContext
) {
  const kind = ctx.kind;
  ...
  if (kind === AxisTickLabelComputingKind.estimate) {
    ...
    ctx.out.noPxChangeTryDetermine.push(
      zrUtil.bind(
        calculateCategoryIntervalTryDetermine,
        null,
        axis,
        interval,
        tickCount
  ...
  const lastInterval = calculateCategoryIntervalDealCache(
    axis,
    interval,
    tickCount
  );
