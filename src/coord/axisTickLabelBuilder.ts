...
function makeCategoryLabelsActually(
  ...
  if (zrUtil.isFunction(optionLabelInterval)) {
    ...
    layout =
      autoInterval || getOptionLabelAutoRotate(labelModel)
        ? makeAutoCategoryLayout(
            axis,
            autoInterval ? undefined : optionLabelInterval
          )
...
export function estimateLabelUnionRect(axis: Axis) {
  ...
  if (scale instanceof OrdinalScale) {
    ...
    const { labels, labelCategoryInterval, rotation } =
      makeCategoryLabelsActually(axis, axisLabelModel);
    const step = layoutScaleStep(
      labels.length,
      axisLabelModel,
...
function getLabelUnionRect(
  ...
  return listCacheSet(
    cache,
    key,
    calculateLabelUnionRect(axis, getLabel, tickCount, step, rotation)
  );
}
...
function calculateLabelUnionRect(
  ...
  for (let i = 0; i < tickCount; i += step) {
    const labelRect = rotateLabelRect(
      labelModel.getTextRect(getLabel(i)),
      rotation,
      padding,
      isHorizontal
    );
...
function makeAutoCategoryLayout(axis: Axis, interval?: number) {
  ...
  return result != null
    && (interval === undefined || result.interval === interval)
    ? result
...
function getCandidateLayouts(axis: Axis) {
  ...
  const labelModel = axis.getLabelModel();
  const labelRotations = normalizeLabelRotations(
    getLabelRotations(labelModel),
    isHorizontal
  );
  ...
  for (const rotation of labelRotations) {
    const { axesIntersection, bounds, offset } = rotateLabel(
      maxLabelSize,
      rotation
    );
    ...
    const interval = calculateLabelInterval(
      unitSize,
      labelSize,
      labelMinDistance
    );
...
function chooseAutoLayout(
  candidateLayouts: InnerAutoLayoutCachedVal[]
): InnerAutoLayoutCachedVal {
  let autoLayout = {
    interval: Infinity,
    rotation: 0,
  };
  ...
  for (const layout of candidateLayouts) {
    if (
      layout.interval < autoLayout.interval
 ...
 * should be implemented in axis.
 */
export function calculateCategoryAutoLayout(
  axis: Axis,
  interval?: number
): InnerAutoLayoutCachedVal {
  const ordinalScale = axis.scale as OrdinalScale;
...
function isHorizontalAxis(axis: Axis): boolean {
  return (
    zrUtil.isFunction((axis as Axis2D).isHorizontal)
...
function getAxisRotate(axis: Axis) {
  return zrUtil.isFunction(axis.getRotate)
    ? axis.getRotate()
    : isHorizontalAxis(axis)
    ? 0
    : 90;
}

function getLabelRotations(
  labelModel: Model<AxisBaseOption['axisLabel']>
): [number, ...number[]] {
...
function normalizeLabelRotations(
...
function getOptionLabelAutoRotate(
  labelModel: Model<AxisBaseOption['axisLabel']>
) {
  const labelRotate = labelModel.get('rotate');
  const autoRotate = labelModel.get('autoRotate');
  return (
    !labelRotate
    && (zrUtil.isArray(autoRotate) ? autoRotate.length > 1 : Boolean(autoRotate))
  );
}
