...
class Grid implements CoordinateSystemMaster {
  ...
  resize(
    ...
    const axesList = [...this._axesList].sort(
      (a, b) => axisOrder(a) - axisOrder(b)
    );
    ...
    if (isContainLabel) {
      each(axesList, function (axis) {
        ...
        if (
          axis.model.get("nameLayout") === "auto" &&
  ...
  convertToPixel(
    ...
    return target.cartesian
      ...
      : target.axis
      ? target.axis.toGlobalCoord(
          target.axis.dataToCoord(value as ScaleDataValue)
        )
      : null;
  }
  /**
