...
define(function (require) {
	...
	function _format(data, labelStyle)
	{
		if (data === null)
			return "null";
			
		var r = /([^;]*)?(?:;([^;]*))?(?:;([^;]*))?(?:;([^;]*))?/;
		var f = r.exec(labelStyle.format);
		// f[1] <-- 正数格式
		// f[2] <-- 负数格式
		// f[3] <-- 零格式
		// f[4] <-- 文本格式
     ...
     */
    function Label(ecTheme, messageCenter, zr, option, myChart){
        // 基类
    ...
    Label.prototype = {
        type : 'label', // 使用硬编码，避免修改太多echarts文件
        ...
        _buildShape : function () {
			...
			{
				if (series[i].type == 'label')
				{
					var serie = self.reformOption(series[i]);
					var labelStyle = zrUtil.merge(serie.labelStyle || {}, labelConfig.labelStyle);
					if (labelStyle.show)
					{
						if (serie.data.length > 0)
						{
							...
							case "object":
								if (data && data.value && data.value !== '-')
								{
									if (data.itemStyle)
									{
										...
										labelStyle = itemStyle.normal.labelStyle;
									}
									if (typeof(data.value) === 'string')
										value = data.value;
									else if (typeof(data.value) === 'number')
										value = _format(data.value, labelStyle);
								}
								break;
							}
							...
							if (value)
							{
								...
								var location = self.getLabelLocation(value, labelStyle, i);
								self.buildLabelBackground(serie.layout.data, labelStyle);
								self.buildLabelValue(value, location, labelStyle);
							}

							var labelLocation = location || { x: 0, y: 0, width: 0, height: 0 };
							if (serie.markText)
							for (var d in serie.markText)
							{
								...
								if (td.text)
								{
									...
									self.buildLabelBackground(td.layout.data, td);
									self.buildLabelValue(td.text, location, td);
								}
							}
						}
					...
					for (var d in serie.markPoint.data)
					{
						...
						v.x = self.getTextDecorationAbsX(v.x, v.position, labelLocation);
						v.y = self.getTextDecorationAbsY(v.y, v.position, labelLocation);
					}
					*/
				}
			}
		 ...
		 * 构建所有标题元素
		 */
		buildLabelValue: function(value, location, labelStyle)
		{
			var self = this;
			var text = value;
			var link = labelStyle.link;
			var font = self.getFont(labelStyle.textStyle);
			...
			var textShape =
			{
				...
				{
					x : x,
					color : labelStyle.textStyle.color,
					text : text,
					textFont : font,
					textAlign : 'left'
					//textBaseline: 'top'
			...
			var baseline = labelStyle.textBaseline || labelStyle.y;
			switch (baseline)
			{
			case 'center':
			case 'middle':
		...
		buildLabelBackground: function(location, labelStyle)
		{
			var self = this;

			//var padding = self.reformCssArray(labelStyle.padding);
			...
			self.shapeList.push(new RectangleShape(
			{
				...
				style :
				{
					...
					brushType : labelStyle.borderWidth === 0
					 ? 'fill' : 'both',
					color : labelStyle.backgroundColor,
					strokeColor : labelStyle.borderColor,
					lineWidth : labelStyle.borderWidth
				}
			}
			));
		},
		...
		doSeriesFormLayout: function(seriesIndex, contentWidth, contentHeight) {
			...
			serie.layout = zrUtil.merge(serie.layout || {}, labelConfig.layout);
		 ...
		 * 根据选项计算标题实体的位置坐标
		 */
		getLabelLocation: function(value, labelStyle, seriesIndex)
		{
			var self = this;
			var padding = self.reformCssArray(labelStyle.padding);
			...
			, totalWidth+padding[1]+padding[3]+labelStyle.borderWidth*2
			, totalHeight+padding[0]+padding[2]+labelStyle.borderWidth*2);
			...
			case 'center':
				...
				break;
			case 'left':
				x = bound.x + padding[3] + labelStyle.borderWidth;
				break;
			case 'right':
			...
			{
			case 'top':
				y = bound.y + padding[0] + labelStyle.borderWidth;
				break;
			case 'bottom':
		 ...
		 * 根据选项计算标题实体的位置坐标
		 */
		getMarkTextLocation: function(text, labelStyle, markIndex, seriesIndex)
		{
			var self = this;
			var padding = self.reformCssArray(labelStyle.padding);
    ...
    zrUtil.inherits(Label, ChartBase);
    zrUtil.inherits(Label, ComponentBase);
    
    // 图表注册
    require('../chart').define('label', Label);
