...
function buildVisuals(
  ...
  each(
    ['color', 'colorAlpha', 'colorSaturation'] as const,
    function (visualName) {
      // Priority: thisNode > thisLevel > parentNodeDesignated > seriesModel
      (designatedVisualItemStyle as any)[visualName] =
        designatedVisual[visualName];
...
function calculateColor(visuals: TreemapVisual) {
  ...
  if (color) {
    ...
    const colorSaturation = getValueVisualDefine(
      visuals,
      'colorSaturation'
    ) as number;
    if (colorSaturation) {
...
function getValueVisualDefine(
  visuals: TreemapVisual,
  name: keyof TreemapVisual
) {
...
function mapVisual(
  ...
  if (mapping) {
    ...
    const colorMappingBy =
      mappingType === 'color' && inner(mapping).drColorMappingBy;
    const value =
