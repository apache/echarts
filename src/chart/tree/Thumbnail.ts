...
class Thumbnail {
  ...
  _treeModel: TreeSeriesModel;
  constructor(containerGroup: graphic.Group) {
      containerGroup.add(this.group);
      this._parent = containerGroup;
  }

  render(seriesModel: TreeSeriesModel, api: ExtensionAPI) {
      ...
      const childrenNodes = (this._parent.children()[0] as graphic.Group).children()[0];
      ...
      for (const node of (childrenNodes as graphic.Group).children()) {
